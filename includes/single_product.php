...
<?php if($counter > 0) {?>
	<div class="full_width pagination">
		<ul style="border-bottom: solid 0px;">
			<li><span><?php print("<b>".$_GET['page']."</b> ".Of.' '.$pages.' ');?></span></li>
			<?php	
				$next_prev = $p->nextPrev($_GET['page'], $pages, "&id=".$_REQUEST['id'].$url.((isset($_REQUEST['cid']))?'&cid='.$_REQUEST['cid']:'').'&pr='.$pr_id.'&bit=1');
				print($next_prev);
			?>
		</ul>
	</div><br />
