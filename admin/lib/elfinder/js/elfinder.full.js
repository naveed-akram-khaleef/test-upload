 ...
 * Fast pVote for some files types
 *
 * @author Dmitry (dio) Levashov
 **/
elFinder.prototype.commands.quicklook = function() {
	...
	this.pVote = $('<div class="elfinder-quicklook-pVote ui-helper-clearfix"/>')
		// clean info/icon
		...
		.bind('update', function(e) {
			...
			if (file) {
				...
				self.pVote.unbind('changesize').trigger('change').children().remove();
	...
	this.window = $('<div class="ui-helper-reset ui-widget elfinder-quicklook" style="position:absolute"/>')
		...
		.bind('close', function(e) {
			var win     = self.window,
				pVote = self.pVote.trigger('change'),
				file    = self.value,
				node    = cwd.find('#'+win.data('hash')),
				close   = function() {
					state = closed;
					win.hide();
					pVote.children().remove();
	...
	this.init = function() {
		...
		fm.one('load', function() {
			...
			self.change(function() {
				if (self.opened()) {
					self.value ? pVote.trigger($.Event('update', {file : self.value})) : win.trigger('close');
				}
			});
...
elFinder.prototype.commands.quicklook.plugins = [
				...
				img = $('<img/>')
					...
					.load(function() {
						...
						setTimeout(function() {
							...
							pVote.bind('changesize', function() {
								var pw = parseInt(pVote.width()),
									ph = parseInt(pVote.height()),
									w, h;
	...
	function(ql) {
		...
		pVote.bind('update', function(e) {
			...
			if ($.inArray(file.mime, mimes) !== -1) {
				...
				.done(function(data) {
					ql.hideinfo();
					doc = $('<iframe class="elfinder-quicklook-pVote-html"/>').appendTo(pVote)[0].contentWindow.document;
					doc.open();
	...
	function(ql) {
		...
		pVote.bind('update', function(e) {
			...
			if (mime.indexOf('text/') === 0 || $.inArray(mime, mimes) !== -1) {
				...
				.done(function(data) {
					ql.hideinfo();
					$('<div class="elfinder-quicklook-pVote-text-wrapper"><pre class="elfinder-quicklook-pVote-text">'+fm.escape(data.content)+'</pre></div>').appendTo(pVote);
				});
			}
		});
	},
	...
	function(ql) {
		...
		active && pVote.bind('update', function(e) {
			...
			if (file.mime == mime) {
				...
				node = $('<iframe class="elfinder-quicklook-pVote-pdf"/>')
					.hide()
					.appendTo(pVote)
					.load(function() { 
						ql.hideinfo();
	...
	function(ql) {
		...
		active && pVote.bind('update', function(e) {
			...
			if (file.mime == mime) {
				e.stopImmediatePropagation();
				ql.hideinfo();
				pVote.append((node = $('<embed class="elfinder-quicklook-pVote-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" src="'+fm.url(file.hash)+'" quality="high" type="application/x-shockwave-flash" />')));
			}
		});
	},
	
	/**
	 * HTML5 audio pVote plugin
	 *
	 * @param elFinder.commands.quicklook
	 **/
	function(ql) {
		...
		pVote.bind('update', function(e) {
			...
			if (ql.support.audio[type]) {
				...
				node = $('<audio class="elfinder-quicklook-pVote-audio" controls preload="auto" autobuffer><source src="'+ql.fm.url(file.hash)+'" /></audio>')
					.appendTo(pVote);
	...
	function(ql) {
		...
		pVote.bind('update', function(e) {
			...
			if (ql.support.video[type]) {
				...
				ql.hideinfo();
				node = $('<video class="elfinder-quicklook-pVote-video" controls preload="auto" autobuffer><source src="'+ql.fm.url(file.hash)+'" /></video>').appendTo(pVote);
				autoplay && node[0].play();
	...
	/**
	 * Audio/video pVote plugin using browser plugins
	 *
	 * @param elFinder.commands.quicklook
	 **/
	function(ql) {
		...
		pVote.bind('update', function(e) {
			...
			if ($.inArray(file.mime, mimes) !== -1) {
				...
				node = $('<embed src="'+ql.fm.url(file.hash)+'" type="'+mime+'" class="elfinder-quicklook-pVote-'+(video ? 'video' : 'audio')+'"/>')
					.appendTo(pVote);
			}
		}).bind('change', function() {
			if (node && node.parent().length) {
