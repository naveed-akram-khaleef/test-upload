<?php
function selected($conditioin){
	$class='';
	if(isset($_REQUEST[$conditioin])){
		$class = ' class="selected" ';
	}
	return $class;
}
function bottom_categories(){
	$res ="
	<div class='full_width category'>
		<ul>";
			$Query = "SELECT c.cat_id, cl.cat_name FROM categories AS c LEFT OUTER JOIN categories_ln AS cl ON c.cat_id=cl.cat_id WHERE c.status_id=1  AND c.cat_parentid=".$_SESSION['vot_cat']." AND cl.lang_id=".$_SESSION['lang_id']." ORDER BY cl.cat_name ASC";
			$count = mysql_num_rows(mysql_query($Query)); 
			$rs = mysql_query($Query);
			if($count>0){
				while($row=mysql_fetch_object($rs)){
					$res .= "<li><a href='index.php?id=".$_REQUEST['id']."&cid=".$row->cat_id."'>".$row->cat_name."</a></li>";
				}
			}
		$res .=
		"</ul>
...
function top_menu($mtype_id, $page_id){
	$res='';
	$Qry=mysql_query("SELECT m.menu_id, m.cnt_id, m.mtype_id, m.menu_parent_id, m.menu_bg, m.status_id, m.menu_link, m.menu_show, m.menu_order, ml.* FROM menu AS m LEFT OUTER JOIN menu_ln AS ml ON m.menu_id=ml.menu_id WHERE m.menu_parent_id=0 AND m.status_id=1 AND m.menu_link=1 AND m.mtype_id IN (".$mtype_id.") AND ml.lang_id=".$_SESSION['lang_id']." ORDER BY m.menu_order ASC");
	if(mysql_num_rows($Qry)>0){
		while($row=mysql_fetch_object($Qry)){
			if($row->menu_id==$page_id){
				$class = 'selected';
			} else {
				$class = '';
			}
			$res .= "<li class=".$class."><a href='index.php?id=".$row->menu_id."'>".$row->menu_title."</a></li>";
		}
	}
	if($mtype_id==1){
		$res .= "<li><a href='includes/logout.php'>".Logout."</a></li>";
	}
	return $res;
}
function getSelCat($pak_id, $plt_id){
	$retVal = "";
	$cnt = 0;
	$rs = mysql_query("SELECT l.*, c.cat_name FROM package_cat_limits AS l INNER JOIN categories AS c ON c.cat_id=l.cat_id WHERE l.pak_id=".$pak_id." AND l.plt_id=".$plt_id." ORDER BY l.cat_id");
	$cntTotal = mysql_num_rows($rs);
	if($cntTotal>0){
		$retVal = "";
		while($row=mysql_fetch_object($rs)){
			$cnt++;
			if($cnt!=$cntTotal){
				$retVal = $retVal . $row->cat_name . ", ";
			}
			else{
				$retVal = $retVal . $row->cat_name;
			}
...
function FillMultiple2($Table, $IDField, $TextField, $SelTbl, $Field1, $Field2, $Field3, $Field3Val, $SelID){
	...
	if (mysql_num_rows($nResult)>=1){
		while ($row=mysql_fetch_row($nResult)){
			$strQuery1="SELECT * FROM $SelTbl WHERE $Field1=$row[0] AND $Field2=$SelID AND $Field3=$Field3Val";
...
function FillMultiple_ParentCats($Table, $IDField, $TextField, $FieldParent, $SelTbl, $Field1, $Field2, $Field3, $Field3Val, $SelID){
	$strQuery="SELECT $IDField, $TextField FROM $Table WHERE $FieldParent=0";
	$nResult =mysql_query($strQuery);
	if (mysql_num_rows($nResult)>=1){
		while ($row=mysql_fetch_row($nResult)){
			$strQuery1="SELECT * FROM $SelTbl WHERE $Field1=$row[0] AND $Field2=$SelID AND $Field3=$Field3Val";
...
function convert_date($date){
	...
	$new_date = date(" M d  ", mktime (0, 0, 0, $arrdate[0], $arrdate[1], $arrdate[2]));
	return $new_date;
}

function convert_date2($date){
	$arrdate = explode("-", $date);
	$new_date = date("D, m M Y", mktime (0, 0, 0, $arrdate[0], $arrdate[1], $arrdate[2]));
	return $new_date;
}

function convert_all_tags($string){
	...
	//$new_string = "'".str_replace("", "", $new_string)."'";
	//$new_string = "'".str_replace("", "", $new_string)."'";
	$new_string   = str_replace("%20", " ", $new_string);
	return $new_string;
}


function imagePropotionate($imgPath, $containerWidth, $containerHeight){
	list($width, $height, $type, $attr) = getimagesize($imgPath);
	$ratio = $width / $height;
	if($ratio >= 1){
		return $imgAttr = " width=".$containerWidth."px ";
	}
	else{
		return $imgAttr = " height=".$containerHeight."px ";
	}
}
...
function avgRating($secID, $objID){
	...
	if(mysql_num_rows($qry) > 0){
		$row = mysql_fetch_object($qry);
		if($row->total_records == 0){
			$retValue = '<span id="serverResponse3">'.Average.' 0 '.Of.' </span><span id="serverResponse4"> 0 '.Rating.'</span>';
		}
		else{
			$avgRate = $row->total / $row->total_records;
			$avg = round($avgRate, 1);
			$retValue = '<span id="serverResponse3">'.Average.' '.$avg.' '.Of.' </span><span id="serverResponse4">'.$row->total_records.' '.Rating.'</span>';
		}
	}
	else{
		$retValue = '<span id="serverResponse3">'.Average.' 0 '.Of.' </span><span id="serverResponse4"> 0 '.Rating.'</span>';
...
function FillSelected2($Table, $IDField, $TextField, $ID, $WHERE){
	echo $strQuery="SELECT $IDField, $TextField FROM $Table WHERE $WHERE ORDER BY $IDField ASC";
	$nResult =mysql_query($strQuery);
	if (mysql_num_rows($nResult)>=1){
		while ($row=mysql_fetch_row($nResult)){
			if($row[0] == $ID){
...
// Display Parents and Childs with Parents disabled
function FillSelected3($Table, $IDField, $TextField, $ID, $parentField){   
...
// Display Parents and Childs with Parents disabled
function FillSelected4($Table, $IDField, $TextField, $ID, $parentField){   
	$strQuery="SELECT $IDField, $TextField FROM $Table WHERE $parentField AND $ID ";
	$nResult =mysql_query($strQuery);
	if (mysql_num_rows($nResult)>=1){
		while ($row=mysql_fetch_row($nResult)){
			echo $strQry="SELECT $IDField, $TextField FROM $Table WHERE $parentField = $row[0]";
			echo "<option disabled='disabled' style='font-weight:bold'>$row[1]</option>";
			$nRs = mysql_query($strQry);
			if (mysql_num_rows($nRs)>=1){
				while ($row1=mysql_fetch_row($nRs)){
...
function sql_str($string){
	$string = str_replace("'", "''", $string); // Converts ' to ' in database, but ' to '' in the static page
...
function delete_record($table, $id, $id_field, $parent_field, $image_field, $main_img, $thumb_img){
	if($parent_field!=''){
		$query = ("SELECT ".$id_field." FROM ".$table." WHERE ".$parent_field." =".$id);
		$res = mysql_query($query);
		while($row = mysql_fetch_row($res)){
			delete_record($table, $row[0], $id_field, $parent_field, $image_field, $main_img, $thumb_img);
		}
	}
	@DeleteFileWithThumb($image_field, $table, $id_field, $id, $main_img, $thumb_img);
	mysql_query("DELETE FROM ".$table." WHERE ".$id_field."=".$id);
}
// function for file deletion
function DeleteFileWithThumb($Field, $Table, $IDField, $ID, $iPath, $tPath){
	...
	if (mysql_num_rows($nResult)>=1){
		$row = mysql_fetch_object($nResult);
		@unlink($iPath.$row->$Field);
		@unlink($iPath.'sml/'.$row->$Field);
		@unlink($iPath.'med/'.$row->$Field);
		@unlink(@$tPath.$row->$Field);
		@unlink($tPath);
	}
}
// function for file deletion
...
function left_side_menu2($Table, $IDField, $TextField, $ID, $parentField){   
	$strQuery="SELECT $IDField, $TextField FROM $Table WHERE $parentField = 0";
	$nResult =mysql_query($strQuery);
	if (mysql_num_rows($nResult)>=1){
		while ($row=mysql_fetch_row($nResult)){
			$strQry="SELECT $IDField, $TextField FROM $Table WHERE $parentField = $row[0]";
			echo "<div class='accordion_header'>$row[1]</div>";
			...
			if (mysql_num_rows($nRs)>=1){
				while ($row1=mysql_fetch_row($nRs)){
					if($row1[0] == $ID){
						echo "<li><a href='products.php?cid=$row1[0]".(!empty($_REQUEST['wholesale'])? "&wholesale=1":"")."'>$row1[1]</a></li>";
					}
					else{
						echo "<li><a href='products.php?cid=$row1[0]".(!empty($_REQUEST['wholesale'])? "&wholesale=1":"")."'>$row1[1]</a></li>";
					}
...
function chkFileExists2($path,$name,$width,$height,$title) {
	$IMG_EXT = unserialize (IMG_EXT);
	$VID_EXT = unserialize (VID_EXT);
	$DOC_EXT = unserialize (DOC_EXT);
	$file_path = '';
	if(file_exists($path.$name)){
		$file_path = $path.$name;
	} else if(file_exists($path.'sml/'.$name)){
		$file_path = $path.'sml/'.$name;
	} else if(file_exists($path.'med/'.$name)){
		$file_path = $path.'med/'.$name;
	} else {
		$file_path = '';
	}
	$fileExt = pathinfo($file_path,PATHINFO_EXTENSION);
	if(empty($file_path) || (pathinfo($file_path,PATHINFO_EXTENSION)=='') || (pathinfo($file_path,PATHINFO_FILENAME)=='')) {
		$flag = 0;
	} else {
	...
	if(file_exists('../files/file_not_found.png')){
		$file_not_found = '../files/file_not_found.png';
	} else {
		$file_not_found = 'files/file_not_found.png';
	}
	if($flag == 1) {
		if(in_array($fileExt, $IMG_EXT)) {
			return "<img class='smallImg' src='".$file_path."' alt='No Image' width='".$width."' height='".$height."' title='".$title."' >";
		}
		else if(in_array($fileExt, $VID_EXT)) {
			return "<a href='".$file_path."' class='player smallImg' id='player' style='display:block;' width='".$width."' height='".$height."' title='".$title."' ></a>";
		}
		else if(in_array($fileExt, $DOC_EXT)) {
			$file_path = "../admin/download_docs.php?name=".$file_path;
			return "<a href='".$file_path."' title='".$title."'> Download </a>";
		} else {
			return "<img class='smallImg' src='$file_not_found' alt='No Image' width='".$width."' height='".$height."' title='".$title."' >";
		}
	} else {
		return "<img class='smallImg' src='$file_not_found' alt='No Image' width='".$width."' height='".$height."' title='".$title."' >";
	}
}
function chkFileExists3($path){
...
function chkFileExists4($file_name){
	if (@file_get_contents('files/products/th/'.$file_name, 0, NULL, 0, 1)) {
		return $_SESSION['site_root'].'files/products/th/'.$file_name;
	}
	if (@file_get_contents('files/products/img/'.$file_name, 0, NULL, 0, 1)) {
		return $_SESSION['site_root'].'files/products/img/'.$file_name;
	}
	else{
		return $_SESSION['site_root'].'files/products/img/no_file.jpg';
	}
}
function chkFileExists5($file_name){
	if (@file_get_contents('files/products/img/'.$file_name, 0, NULL, 0, 1)) {
		return $_SESSION['site_root'].'files/products/img/'.$file_name;
	}
	if (@file_get_contents('files/products/th/'.$file_name, 0, NULL, 0, 1)) {
...
function chkFileExists6($file_name){
	if (@file_get_contents('files/products/file/'.$file_name, 0, NULL, 0, 1)) {
		return $_SESSION['site_root'].'files/products/file/'.$file_name;
	}
	if (@file_get_contents('files/products/img/'.$file_name, 0, NULL, 0, 1)) {
		return $_SESSION['site_root'].'files/products/img/'.$file_name;
	}
...
function home_products($cat_id, $limit, $sort_by, $page_id, $lang_id){
	$res='';
	$counter=0;
	if($sort_by=='na'){
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY pr.pr_id DESC LIMIT ".$limit." ";
	} elseif($sort_by=='tr'){
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id, ( SELECT SUM( v.rate ) FROM votes AS v WHERE v.section_id=1 AND v.pro_id=pr.pr_id LIMIT 1 ) as res FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY res DESC LIMIT ".$limit." ";			
	} elseif($sort_by=='mv'){
		$Query = "SELECT pr.pr_id, pl.pr_title, pr.pr_hits, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY pr.pr_hits DESC LIMIT ".$limit." ";
	} elseif($sort_by=='fr'){
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_id." AND pr.is_featured=1 AND pl.lang_id=".$lang_id." ORDER BY pr.pr_id LIMIT ".$limit." ";
	} else{
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY pr.pr_id ASC LIMIT ".$limit." ";
	}
	...
	if($total>0){
		while ($row=mysql_fetch_object($Qry)){
			...
			//$atr = imagePropotionate($imgPath, 203, 121);
			$res .= "<div class='col'><div class='thumbs'><div class='image'><a href='index.php?id=".$page_id."&pr=".$row->pr_id."'><img src='".$imgPath."' /></a></div><div class='caption'><a href='index.php?id=".$page_id."&pr=".$row->pr_id."'>".limit_text($row->pr_title,15)."</a></div></div></div>";
		}
	} else {
		$res .= '<div class="thumbs"><div class="caption"> There are no products in this category. </div></div>';
	}
	return $res;
}

function home_products2($cat_id, $cat_parent_id, $limit, $sort_by, $page_id, $lang_id){
	$res='';
	$counter=0;
	if($sort_by=='na'){
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_parent_id." AND c.cat_id=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY pr.pr_id DESC LIMIT ".$limit." ";
	} elseif($sort_by=='tr'){
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id, ( SELECT SUM( v.rate ) FROM votes AS v WHERE v.section_id=1 AND v.pro_id=pr.pr_id LIMIT 1 ) as res FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_parent_id." AND c.cat_id=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY res DESC LIMIT ".$limit." ";			
	} elseif($sort_by=='mv'){
		$Query = "SELECT pr.pr_id, pl.pr_title, pr.pr_hits, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_parent_id." AND c.cat_id=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY pr.pr_hits DESC LIMIT ".$limit." ";
	} elseif($sort_by=='fr'){
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_parent_id." AND c.cat_id=".$cat_id." AND pr.is_featured=1 AND pl.lang_id=".$lang_id." ORDER BY pr.pr_id LIMIT ".$limit." ";
	} else{
		$Query = "SELECT pr.pr_id, pl.pr_title, prf.prf_thumb, prf.prf_file, c.cat_id FROM products AS pr LEFT OUTER JOIN categories AS c ON pr.cat_id=c.cat_id LEFT OUTER JOIN pr_files AS prf ON pr.pr_id=prf.pr_id LEFT OUTER JOIN products_ln AS pl ON pr.pr_id=pl.pr_id WHERE pr.status_id=1 AND c.cat_parentid=".$cat_parent_id." AND c.cat_id=".$cat_id." AND pl.lang_id=".$lang_id." ORDER BY pr.pr_id ASC LIMIT ".$limit." ";
	}
	$Qry = mysql_query($Query);
	$total = mysql_num_rows($Qry);
	if($total>0){
...
function avg_rating($cat_id, $pr_id){
	$total_ratings = 0;
	$total_vots = 0;
	$avg = 0.0;
	$qry1 = mysql_query("SELECT COUNT( 1 ) AS total_records, SUM( rate ) AS total FROM votes WHERE section_id=".$cat_id." AND pro_id=".$pr_id." ");
